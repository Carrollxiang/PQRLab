...
def get_pb_data(line):
    ...
    output_pattern = re.findall(r"0b[\w\s]+",re.findall(r"0b[\w\s]+\,", line)[0])[0]  # 2进制
    # output_pattern = re.findall(r"0x[a-zA-Z0-9]+", line)[0]  # 16进制
    ...
    if len(inst) != 0:
        ...
        if len(inst_data) != 0:
            inst_data = re.findall(r"[0-9]+", inst_data[0])[0]
        else:
            inst_data = "0"
    else:
        inst = "0"
    ...
    if len(comment) != 0:
        comment = comment[0]
        return "pb_inst_pbonly(" + output_pattern + "," + inst + "," + inst_data + "," + time + "*" + unit + ")" + " # " + comment
    else:
        return "pb_inst_pbonly("+output_pattern+","+inst+","+inst_data+","+time+"*"+unit+")"